...
describe 'Blacklight Range Limit' do
  it 'should show the range limit facet' do
    ...
    expect(page).to have_selector 'label[for="range_pub_date_drsi_begin"]',
                                  text: I18n.t('blacklight.range_limit.range_begin_short')
    expect(page).to have_selector 'label[for="range_pub_date_drsi_end"]',
                                  text: I18n.t('blacklight.range_limit.range_end_short')
  ...
  it 'should provide distribution information' do
    ...
    expect(page.find('a.facet-select',
                     text: '1500 to 1599').ancestor('li')).to have_selector('span.facet-count', text: '0')
    expect(page).to have_selector('a.facet-select', text: '2000 to 2008')
    expect(page.find('a.facet-select',
                     text: '2000 to 2008').ancestor('li')).to have_selector('span.facet-count', text: '12')
  end
  ...
  it 'should limit appropriately' do
    ...
    within '.blacklight-pub_date_drsi' do
  ...
  context 'when I18n translation is available' do
    before do
      I18n.backend.store_translations(:en,
                                      blacklight: { search: { fields: { facet: { pub_date_drsi: 'Publication Date I18n' } } } })
    end

    after do
      I18n.backend.store_translations(:en,
                                      blacklight: { search: { fields: { facet: { pub_date_drsi: 'Publication Date Sort' } } } })
    end
  ...
  context 'when on a zero results found page' do
    it 'should not render the range limit facet' do
      ...
      expect(page).to_not have_selector '#facets .facet-limit.blacklight-pub_date_drsi'
      expect(page).to_not have_selector 'input#range_pub_date_drsi_begin'
      expect(page).to_not have_selector 'input#range_pub_date_drsi_end'
    end
  end
end
...
describe 'Blacklight Range Limit with configured input labels' do
  before do
    ...
    CatalogController.configure_blacklight do |config|
      config.add_facet_field 'pub_date_drsi', range: true
  ...
  it 'should show the range limit facet with configured labels' do
    ...
    expect(page).to have_selector 'input#range_pub_date_drsi_begin'
    expect(page).to have_selector 'input#range_pub_date_drsi_end'
  end
end
