...
describe 'Run through with javascript', js: true do
  ...
  it 'basically works' do
    ...
    within '.facet-limit.blacklight-pub_date_drsi' do
      expect(page).to have_css('canvas')

      # min/max in actual results are filled in inputs
      expect(find('input#range_pub_date_drsi_begin').value).to be_present
      expect(find('input#range_pub_date_drsi_end').value).to be_present
      ...
      find('input#range_pub_date_drsi_begin').set(start_range)
      find('input#range_pub_date_drsi_end').set(end_range)
    ...
    within '.facet-limit.blacklight-pub_date_drsi' do
      ...
      expect(find('input#range_pub_date_drsi_begin').value).to eq start_range
      expect(find('input#range_pub_date_drsi_end').value).to eq end_range
  ...
  context 'for single dates' do
    it 'does not show chart or facet list' do
      ...
      last_date = nil
      within '.facet-limit.blacklight-pub_date_drsi' do
        last_date = find('input#range_pub_date_drsi_begin').value

        find('input#range_pub_date_drsi_begin').set(last_date)
        find('input#range_pub_date_drsi_end').set(last_date)
        click_button 'Apply limit'
      end
  ...
  context 'open-ended range' do
    it 'can search' do
      ...
      within '.facet-limit.blacklight-pub_date_drsi' do
        find('input#range_pub_date_drsi_begin').set('')
        find('input#range_pub_date_drsi_end').set(end_range)
        click_button 'Apply limit'
      end
  ...
  context 'submitted with empty boundaries' do
    it 'does not apply filter' do
      ...
      within '.facet-limit.blacklight-pub_date_drsi' do
        find('input#range_pub_date_drsi_begin').set('')
        find('input#range_pub_date_drsi_end').set('')
        click_button 'Apply limit'
      end
  ...
  context 'when assumed boundaries configured' do
    around do |example|
      original = CatalogController.blacklight_config.facet_fields['pub_date_drsi'].range_config
      CatalogController.blacklight_config.facet_fields['pub_date_drsi'].range_config = original.merge({
                                                                                                        assumed_boundaries: 1900...2100
                                                                                                      })

      example.run

      CatalogController.blacklight_config.facet_fields['pub_date_drsi'].range_config = original
    end
