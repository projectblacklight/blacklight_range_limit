...
class TestAppGenerator < Rails::Generators::Base
  ...
  def run_bl7_jsbundling_fixup
    return unless File.exist?('package.json') && Blacklight::VERSION.split('.').first == '7'

    say_status('warning', 'BlacklightRangeLimit: Blacklight 7.x package.json-based Test App fixup',
               { color: :yellow })
  ...
  def run_bl8_importmaps_fixup
    ...
    unless Pathname(destination_root).join('config/importmap.rb').exist? && Blacklight::VERSION.split('.').first == '8'
      return
    end

    # BL's importmap setup annoyingly uses a different name for the BL package than their package.json setup
    gsub_file('app/javascript/application.js', 'import Blacklight from "blacklight-frontend";',
  ...
  def inject_into_catalog_controller
    inject_into_file 'app/controllers/catalog_controller.rb', after: /config.add_facet_field 'format'.*$/ do
      "\n    config.add_facet_field 'pub_date_drsi', label: 'Publication Date Sort', range: true"
    end
  end
end
